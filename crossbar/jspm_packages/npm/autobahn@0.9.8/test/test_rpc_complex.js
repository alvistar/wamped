...
exports.testRpcComplex = function(testcase) {
  ...
  connection.onopen = function(session) {
    ...
    autobahn.when.all(pl1).then(function() {
      ...
      var d = session.call('com.myapp.add_complex', [2, 3, 4, 5]).then(function(res) {
        test.log("Result: " + res.kwargs.c + " + " + res.kwargs.ci + "i");
      });
      d = d.then(function() {
        return session.call('com.myapp.split_name', ['Homer Simpson']).then(function(res) {
      ...
      for (var i = 0; i < params.length; ++i) {
        (function(i, args, kwargs) {
          d = d.then(function() {
            return session.call('com.myapp.echo_complex', args, kwargs).then(function(res) {
